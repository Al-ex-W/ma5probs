...
def getvalue(num, base) # översätter tal från vilken bas som helst till 10 bas
    ...
    i = (0-amount_of_decimals) # Startvärde för exponenten, ökar för varje nytt värde från $inputarr, hänvisa till pappret.
...
def translatevalue(newbase) # Översätter tal i tiobas till vad som helst
    ...
    while ($numsum / newbase) >= 1 # kollar om $numsum går att representera med (ytterligare) 1 tecken från den nya basen, om den behöver fler, ska den köra loopen, hänvisa till pappret.
       $newvalue += $numarr[($numsum % newbase)] # konkatenerar $newvalue (talet i den nya basen) med resten som blir när talet i tiobas delas med den nya basen, hänvisa till papper. 
       $numsum = $numsum / newbase # delar numsum med basen för att sedan fortsätta på nästa tal i den nya basen, hänvisa till papper
    end
    ...
    if all_decimals != 0 #Har talet decimaler?
        all_decimals = ("0." + "#{all_decimals}").to_f # gör om decimalerna till riktiga decimaler i float.
        ...
        while all_decimals != 0 && i < 6 #om det finns decimaler kvar att beräkna eller om den har loopat mindre än 6 gånger så körs loopen
            newdecimals << $numarr[((all_decimals * newbase).to_i)] # multiplicerar (de återstående) decimelerna. Heltalet som blir som resultat är decimalens värde. Tecknet för detta värde hämtas ur $numarr, se papper.
            all_decimals = all_decimals * newbase - ((all_decimals * newbase).to_i) # sätter all_decimals till det återstående värdet på decimaltalen, som måste visas med ännu mindre decimaler, se papper
            i+=1
        end
